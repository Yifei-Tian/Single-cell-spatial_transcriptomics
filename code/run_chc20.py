...
功能：对 CHC20 Visium 切片执行完整的预处理 + Cell2location 反卷积流程，
      并自动调用空间生态位分析（Step 2），结果保存到 results/CHC20/。
...
输出目录：
    results/CHC20/                   ← Cell2location 反卷积结果
    results/CHC20/spatial_niche/     ← 空间生态位分析结果
================================================================================
"""
...
# ── 项目根目录（run_chc20.py 位于 code/ 下，向上一级为项目根）
_REPO_ROOT = Path(__file__).resolve().parent.parent

# ── 数据集专属配置 ────────────────────────────────────────────────────────────
SAMPLE1_NAME   = "CHC20"
SAMPLE2_NAME   = "CHC23"          # 原配对验证切片（可不传）
PATH_SCRNA     = _REPO_ROOT / "data" / "scRNA_reference.h5ad"
PATH_SAMPLE1   = _REPO_ROOT / "data" / "CHC20_Visium"
PATH_SAMPLE2   = _REPO_ROOT / "data" / "CHC23_Visium"
OUTPUT_DIR     = _REPO_ROOT / "results" / SAMPLE1_NAME


def parse_args():
    p = argparse.ArgumentParser(description="CHC20 完整分析流程（Step 1 + Step 2）")
...
def run_step1():
    ...
    # 将 pipeline/ 加入 sys.path，使 run_preprocessing 可正常 import utils/
    ...
    preprocessing_main(
        ...
        path_sample2=PATH_SAMPLE2,  # 同时处理 CHC23 配对验证切片
...
def run_step2():
    """运行 Step 2：空间生态位分析（通过命令行调用，保持参数灵活性）"""
    ...
    if not adata_path.exists():
        ...
        print("       Please run Step 1 first: python code/run_chc20.py --step1-only")
        sys.exit(1)
    ...
    print(f"[run_chc20] Running Step 2: {' '.join(cmd)}")
