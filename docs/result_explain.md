...
气泡大小表示在某一个特定的细胞簇（x轴的数字）中，**有多少比例的细胞**表达了该基因。气泡越大，说明该群体中表达这个基因的细胞占比越高（例如最大的气泡代表接近 100% 的细胞都表达）。如果只是一个针尖大的小点，说明这个群体里几乎没有细胞表达该基因。

颜色深浅表示在那些表达了该基因的细胞中，**基因表达量的平均水平有多高**。颜色越蓝、越深（靠近色条的 7.5），说明该基因的表达量极高；颜色越白、越浅（靠近 2.5），说明表达量较低。
...
- 动作 5：综合打分（Score） 除了上面的粗颗粒度聚类，流程还为**每一个** spot 计算了一个连续的 `immunosuppressive_niche_score`（5项 Z-score 的加权和）。
- 动作 6：切分阈值（High vs Low） 通过设定阈值（超过 80 百分位），硬性划分出 `niche_high` 区域。**这一步是与前面的聚类解耦的**。它的好处是，即使某个 spot 没有被分进最高分的 Cluster，但只要它自身的抑制评分足够高，依然会被纳入下游分析，保证了统计效力。
