...
\begin{tikzpicture}[
    ...
    box/.style={
        ...
        align=center, fill=blue!6, inner sep=4pt
    },
    box2/.style={
        rectangle, rounded corners=2pt, draw=black!70, thick,
        minimum width=3.4cm, minimum height=1.05cm,
        align=center, fill=orange!10, inner sep=4pt
    },
    box3/.style={
        rectangle, rounded corners=2pt, draw=black!70, thick,
        minimum width=3.4cm, minimum height=1.05cm,
        align=center, fill=green!10, inner sep=4pt
    },
...
% ---- 第一行：数据与反卷积 ----
\node[box] (sc) {scRNA-seq 参考图谱\\（GSE149614）\\Treg 亚群鉴定};
\node[box, right=of sc] (st) {Visium 空间转录组\\HCC4R / CHC20\\质控与预处理};
\node[box2, right=of st] (c2l) {Cell2location\\贝叶斯反卷积\\spot 级细胞丰度};

% ---- 第二行：生态位识别 ----
\node[box2, below=of c2l] (niche) {邻域组成 $k$-NN 图\\Leiden 聚类\\免疫抑制生态位识别};
\node[box2, left=of niche] (sig) {两层特征基因筛选\\Wilcoxon/AUC + 先验基因集\\生态位签名基因};

% ---- 第三行：机制、验证、临床 ----
\node[box3, below=of sig] (lr) {配体-受体通讯分析\\CXCL12--CXCR4 等\\机制层面解析};
\node[box3, right=of lr] (cross) {CHC20 跨切片验证\\双模块评分复现\\空间层面验证};
\node[box3, right=of cross] (tcga) {TCGA-LIHC 队列\\ssGSEA + 多变量 Cox\\临床预后验证};

% ---- 连接箭头 ----
\draw[arrow] (sc) -- (st);
\draw[arrow] (st) -- (c2l);
\draw[arrow] (c2l) -- (niche);
\draw[arrow] (niche) -- (sig);
\draw[arrow] (sig) -- (lr);
\draw[arrow] (sig.east) -- (cross.north);
\draw[arrow] (sig.east) to[out=20, in=160] (tcga.north);

% ---- 阶段标注 ----
\node[stagelabel, above=0.15cm of sc] {阶段一：单细胞参考构建};
\node[stagelabel, above=0.15cm of c2l] {阶段二：空间反卷积};
\node[stagelabel, below=0.7cm of niche] {阶段三：生态位识别与特征基因提取};
\node[stagelabel, below=0.7cm of lr] {阶段四：机制解析};
\node[stagelabel, below=0.7cm of cross] {阶段五：跨样本 \& 临床验证};

\end{tikzpicture}
\caption{整体分析流程图。流程分为五个阶段：（一）基于 scRNA-seq 构建细胞类型参考图谱并鉴定 Treg 亚群；（二）以 Cell2location 将细胞类型丰度反卷积映射至 Visium 空间切片；（三）基于空间邻域组成的无监督聚类识别免疫抑制生态位，并以两层筛选策略提取生态位特征基因签名；（四）通过配体-受体通讯分析解析 Treg--TAM--CAF 轴的分子机制；（五）分别在独立切片 CHC20 与 TCGA-LIHC 大队列中完成空间层面与临床层面的双重验证。}
\label{fig:workflow}
\end{figure}
...
为验证上述邻域参数选择不会主导结论，本研究设计了不依赖硬阈值截断的连续型敏感性分析。分别以 $k$-NN 邻居数 $k \in \{10, 15, 20\}$ 及半径倍增系数 $\text{radius\_multiplier} \in \{1.0, 1.25, 1.5\}$ 两种邻域定义方式对综合 niche 评分（公式~\ref{eq:combined_rank} 所依据的四维 Z-score 加总）进行参数扰动，以主分析参数（$k=15$）下的评分向量作为参考基准，采用两个互补的连续型指标量化每次扰动后评分分布与参考基准的一致性，从而完全避免早期版本中因分位数硬截断（如仅比较 $k$-NN 前 20\% 高分 spot 集合的 Jaccard 相似度）所带来的边界敏感性问题：
...
导致两组间均值差异（log2FC）被严重稀释，传统"仅依赖 Wilcoxon 检验 + log2FC 阈值"的
...
第一层通过统计检验对全基因组进行数据驱动的排序与筛选，
第二层针对先验已知的功能基因集进行靶向核查与强制补全，
二者共同构成最终的签名基因列表。

第一层（Layer 1：Wilcoxon + BH-FDR + AUC + 检出率综合排序）：
...
第二层（Layer 2：先验功能基因集的靶向 AUC 检验与强制补全）：
...
对每对配体-受体 $(l, r)$，以 CP10K+log1p 标准化表达量 $E_l$、$E_r$ 的乘积
作为通讯强度代理指标：
...
分别计算 niche\_high 与 niche\_low 组的均值 LR\_score，并以

\begin{equation}
\log_2\text{FC} = \log_2\left(\frac{\overline{\text{LR}}_{\text{high}} + \varepsilon}{\overline{\text{LR}}_{\text{low}} + \varepsilon}\right), \qquad \varepsilon = 10^{-6}
\label{eq:lr_log2fc}
\end{equation}

量化生态位内的通讯富集倍数，其中 $\varepsilon$ 为避免分母为零而引入的极小平滑常数。
...
niche\_high（$\geq$90百分位）与 niche\_low 组的差异基因火山图（Figure 4A）揭示了
...
Layer 2 先验基因集的检出率差值（$\Delta$frac）和 AUC 指标对此进行了专项补偿~\citep{luecken2019current}。
...
\begin{lstlisting}[caption={Layer 1 block 级 Wilcoxon + AUC + 检出率综合排序，Layer 2 先验基因集强制合并}]
# Layer 1: block-level Mann-Whitney U + AUC + delta_frac
...
# 检出率差值（解决细胞稀释效应导致的均值 FC 低估）
frac_high = (high_expr > 0).mean(axis=0)
frac_low  = (low_expr  > 0).mean(axis=0)
...
# signature_rank_score 加权综合排序
score = 0.35 * minmax(log2fc) + 0.35 * minmax(delta_frac) + 0.30 * minmax(auc)

# Layer 2: 先验功能基因集 AUC + delta_frac 检验，强制合并入签名列表
for gene in prior_gene_set:
    stat, p = mannwhitneyu(high_expr[gene], low_expr[gene], alternative='greater')
    auc_g = stat / (len(high_expr) * len(low_expr))
    dfrac_g = (high_expr[gene] > 0).mean() - (low_expr[gene] > 0).mean()
    if auc_g > 0.6 and dfrac_g > 0.05 and gene not in non_specific_genes:
        forced_signature_genes.add(gene)
\end{lstlisting}
