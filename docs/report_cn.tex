...
与 Ma et al.~\cite{ma2019comprehensive} 的肝癌空间转录组研究相比，本研究的创新点在于引入了 L-R 通讯分析和 CHC23 多切片一致性验证，将单一切片的描述性发现提升为跨样本的机制性结论。两张独立切片（CHC20 和 CHC23）在 Treg 富集区域、CAF 分布模式和免疫抑制 niche 特征上的高度一致性，进一步增强了结论的生物学可信度。
