...
肝细胞癌是全球第六大恶性肿瘤，每年新发病例超过90万，死亡病例约83万，
...
HCC 微环境中的免疫抑制由多种细胞协同构建。
Treg 以 CD4$^+$CD25$^+$FOXP3$^+$ 为经典表型，
通过分泌 IL-10、TGF-$\beta$ 和 IL-35，以及表达 CTLA-4、TIGIT、LAG-3 等检查点分子，
直接抑制效应性 T 细胞与 NK 细胞的抗肿瘤活性~\citep{sakaguchi2020regulatory}；
单细胞转录组研究表明 Treg 在肝癌组织中显著富集，
Treg 与 CD8$^+$ T 细胞的比值是独立预后因子~\citep{zheng2017landscape}。
以 M2 极化表型（CD163$^+$MRC1$^+$）为主的 TAM 通过 CCL22--CCR4 和 CXCL12--CXCR4 轴
...
以 FAP、ACTA2 和 POSTN 为标志的 CAF 则通过分泌 CXCL12 招募免疫抑制细胞，
...
尽管上述技术进步令人振奋，现有 HCC 研究仍普遍缺少三个维度的整合：
在保留空间信息的前提下，将 Treg--TAM--CAF 轴的共定位从描述性层面提升至配体-受体通讯机制层面；
采用数据驱动（而非经验阈值）的方法识别免疫抑制空间生态位；
...
scRNA-seq 参考数据来自 GEO 数据库（登录号：GSE149614），
涵盖肝细胞（Hepatocyte）、T/NK 细胞、B 细胞、髓系细胞（Myeloid）
和成纤维细胞（Fibroblast）等细胞类型。
参考当前 scRNA-seq 分析最佳实践~\citep{luecken2019current}，
过滤总表达量低于200的低质量细胞及在少于10个细胞中表达的基因。
由于 Cell2location RegressionModel 的贝叶斯框架已内置对测序深度和批次效应的建模，
此阶段不执行 library size 归一化或 log1p 变换，直接以原始 count 矩阵作为输入。

空间转录组数据由 10x Genomics Visium 平台生成，包含两张肝癌组织切片：
HCC4R 作为主分析切片，CHC20 作为独立验证切片。
参照空间转录组分析最佳实践~\citep{williams2022introduction,palla2022squidpy}，
保留总 UMI 计数在5000--35000之间的 spot，
过滤线粒体基因比例超过20\%的 spot，
并保留在至少10个 spot 中有表达的基因。
预后验证数据来自 TCGA-LIHC 队列（约370例患者），
通过 TCGAbiolinks R 包~\citep{colaprico2016tcgabiolinks} 从 GDC 数据库下载 HTSeq-FPKM 格式表达矩阵及临床随访信息。
...
基于 $k=15$ 近邻构建空间邻接图，
对每个 spot $i$ 计算其 $k$ 个近邻的细胞类型比例均值作为邻域组成向量 $\mathbf{n}_i$：
...
$k=15$ 沿用 Schürch 等人~\citep{schurch2020coordinated} 在结直肠癌研究中的参数，
并通过 $k \in \{10, 15, 20\}$ 的敏感性分析验证结论的参数稳健性。
以邻域组成向量为特征，在特征空间中构建 $k$-NN 图，
运行 Leiden 社区检测算法（分辨率=0.5）识别细胞邻域类型（cellular neighborhoods）；
与 KMeans 相比，Leiden 无需预先指定聚类数、且理论上保证了聚类连通性~\citep{traag2019leiden}。

对每个 Leiden cluster，计算 Treg 比例、Myeloid 比例、
Fibroblast 比例和免疫抑制基因模块评分（见下）四个功能维度的均值，
...
敏感性分析采用 Spearman 秩相关系数（$\rho$）和连续型 Weighted Jaccard 相似度
...
对 niche\_high（$\geq$85百分位）vs niche\_low 组执行 Wilcoxon 检验+BH-FDR 校正~\citep{mann1947test}；
...
第三层（Layer 3）：通过 Gini 不均等指数~\citep{yitzhaki1979relative}（阈值 Gini $>$ 0.3 且 log2FC $>$ 0）
捕获局灶性高表达的稀有免疫基因，
