...
\lstset{
    ...
    extendedchars=false,      % 关键：关闭 listings 对高位字符的默认处理，允许中文
    keepspaces=true,          % 保持空格
    basicstyle=\small\ttfamily, % 使用等宽字体
    commentstyle=\color{gray},
    % 如果你希望中文注释能正常显示，最好显式指定逃逸字符（见方法2）
}
...
{\color{red}（目前这个版本这三类细胞有点太割裂了，想改成既简单介绍一下细胞，又体现出免疫抑制的机制）}

肿瘤微环境是一个由肿瘤细胞、免疫细胞、基质细胞及细胞外基质共同构成的复杂生态系统。
其中，在肿瘤微环境中起核心作用的细胞有调节性 T 细胞（Regulatory T cells, Treg）、肿瘤相关巨噬细胞（TAM）和肿瘤相关成纤维细胞（CAF）。

Treg 是免疫抑制微环境的核心节点细胞。
Treg 以 FOXP3 为关键转录因子，以 $\text{CD4}^{+}\text{CD25}^{+}\text{FOXP3}^{+}$ 为经典表型，
...
\subsection{空间转录组学的前沿进展}
传统 bulk RNA-seq 将整个组织匀浆后测序，丢失了细胞类型和空间位置信息；
单细胞转录组（scRNA-seq）虽解析单细胞转录特征，能够获得细胞类型的相关信息，
但在组织解离时不可避免地破坏细胞间的空间关系~\citep{luecken2019current}。
10x Genomics Visium 空间转录组平台则能在保留组织原位空间结构的前提下，
对每个空间位点（spot，直径约55\,$\mu$m，覆盖约1--10个细胞）进行全转录组测序~\citep{williams2022introduction}。
这一技术使研究人员能够直接观测细胞在组织中的空间分布模式，
揭示哪些细胞类型在哪些空间区域共定位（co-localization），
以及局部微环境的基因表达状态。

然而，Visium 技术目前的空间分辨率尚未达到单细胞水平，每个 spot 实际上是多种细胞类型转录信号的混合~\citep{williams2022introduction}。因此，必须结合 scRNA-seq 参考数据通过计算反卷积方法，将每个 spot 的混合信号分解为各细胞类型的相对丰度估计。

当前主流的空间反卷积工具可大致分为贝叶斯层次模型类（Cell2location, RCTD）、空间条件概率类（CARD）和深度学习类（Tangram）这三类方法。

Cell2location~\cite{kleshchevnikov2022cell2location} 基于负二项贝叶斯层次模型，以单细胞数据为参考，在全基因组范围内同时估计多种细胞类型的绝对丰度，并自然地提供后验不确定性量化。其显著优势在于：能够同时建模技术噪声（检测效率、批次效应）和生物变异；对稀有细胞类型（如 Treg，典型比例 $<$ 5\%）有更高的检测灵敏度；在复杂组织结构（如异质性强的肿瘤边缘区域）中表现更为稳健。
...
一致表明，在多细胞类型混合、稀有细胞类型检测的场景中，Cell2location 性能优异，是肿瘤免疫微环境空间解析的首选工具。
